...
// Track WhatsApp button clicks → Supabase (so landlords see ROI later)
document.addEventListener('click', (e) => {
  const a = e.target.closest('a.btn-wa, a.btn-call');
  if (!a) return;
  const card = a.closest('[data-listing-id]');
  const listingId = card?.dataset.listingId;
  if (!listingId || !window.nodalaliSupabase) return;
  // Fire-and-forget; don't block the link from opening
  window.nodalaliSupabase.raw.from('whatsapp_clicks').insert({
...
async function openReport(listingId) {
  ...
  // Try Supabase directly (works without backend)
  if (window.nodalaliSupabase) {
    const { error } = await window.nodalaliSupabase.raw.from('reports').insert({
      listing_id: listingId, reason
    });
    if (!error) { toast('✓ Asante! Ripoti yako imehifadhiwa.'); return; }
    console.warn('Supabase report failed:', error);
  }
  try {
...
// ============== POST LISTING ==============
// Low-friction: send listing to admin WhatsApp without signup/payment
document.getElementById('sendWhatsAppBtn').addEventListener('click', () => {
  const f = document.getElementById('listingForm');
  const fd = new FormData(f);
  const required = ['title','city','area','bedrooms','bathrooms','price','type','whatsapp'];
  for (const k of required) {
    if (!fd.get(k)) { toast('Jaza: ' + k); f.querySelector(`[name="${k}"]`).focus(); return; }
  }
  if (!/^255[67][0-9]{8}$/.test(fd.get('whatsapp'))) {
    toast('Namba isiyo sahihi — anza na 255'); return;
  }
  const msg = [
    '🏠 *TANGAZO JIPYA - NODALALI*',
    '',
    `*Kichwa:* ${fd.get('title')}`,
    `*Mji:* ${fd.get('city')}`,
    `*Eneo:* ${fd.get('area')}`,
    `*Aina:* ${fd.get('type')}`,
    `*Vyumba:* ${fd.get('bedrooms')} | *Bafu:* ${fd.get('bathrooms')}`,
    `*Bei:* TZS ${Number(fd.get('price')).toLocaleString()}/mwezi`,
    `*WhatsApp ya mwenye nyumba:* ${fd.get('whatsapp')}`,
    '',
    `*Maelezo:* ${fd.get('description') || '(hakuna)'}`,
    fd.get('lat') ? `*Eneo (GPS):* ${fd.get('lat')}, ${fd.get('lng')}` : '',
    '',
    '_Tafadhali pakia tangazo hili kwenye Nodalali. Asante._'
  ].filter(Boolean).join('\n');
  const url = 'https://wa.me/255747200123?text=' + encodeURIComponent(msg);
  window.open(url, '_blank');
  toast('✅ WhatsApp imefunguliwa — bonyeza Send');
});
...
async function loadSupabaseListings() {
  if (!window.nodalaliSupabase) return false;
  try {
    const rows = await window.nodalaliSupabase.listListings();
    ...
    console.log(`✅ Loaded ${rows.length} listings from Supabase`);
    toast(`📡 Supabase: ${rows.length} listings`);
    renderListings();
    refreshMapMarkers();
    if (browseMapInstance) refreshMapMarkers(browseMapInstance, browseMapMarkers);
    return true;
  } catch (e) {
    console.warn('Supabase load failed, will try Firestore:', e);
    return false;
  }
}

// Startup: try Supabase first, fall back to Firestore
(async () => {
  const ok = await loadSupabaseListings();
  if (!ok && useFirebase && db) loadFirebaseListings();
})();
