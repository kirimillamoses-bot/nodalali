...
(function () {
  ...
  window.nodalaliSupabase = {
    ...
    // Upload an image (Blob/File) to the `listings` Storage bucket.
    // Returns the public URL on success, or null if the bucket isn't configured.
    async uploadListingImage(blob, opts = {}) {
      try {
        const ext = (blob.type && blob.type.split('/')[1]) || 'jpg';
        const userId = (await sb.auth.getUser()).data?.user?.id || 'anon';
        const name = `${userId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
        const { error } = await sb.storage.from('listings').upload(name, blob, {
        ...
        const { data } = sb.storage.from('listings').getPublicUrl(name);
